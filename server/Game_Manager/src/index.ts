...
const PORT_EXPRESS_CONTROLLER_GAMEMANAGER: number = parseInt(
  `${process.env.PORT_EXPRESS_CONTROLLER_GAMEMANAGER}`,
);
...
const matchSettings = async () => {
  ...
  timer_duration = (response?.matchLength as unknown as number)
    ? (response?.matchLength as unknown as number)
    : parseInt(`${process.env.TIMER_DURATION}`);
...
const gameCycle = setInterval(async () => {
  if (game_state == GAME_STATE.NOT_PLAYING) {
    ...
    if (queue.length == numPlayers * 2) {
      if (robots_ready) {
        ...
        console.log("[MATCH] Robots ready - entering confirmation block");
  ...
  //else if currently confirming
    ...
    console.log(
      `TIMER: ${timer} | ${players[0]["username"]} vs ${players[1]["username"]}`,
    );
    timer--;
    if (timer == 0) {
  ...
  (ws: any, request: IncomingMessage, username: string, user_id: string) => {
    ...
    ws.on("message", (data: any) => {
      ...
      //when user is sending confirmation that they're ready
        ...
        const { password, accepted }: { password: string; accepted: boolean } =
          payload;
...
server_wss_CLIENT_GM.on("upgrade", async (request, socket, head) => {
  ...
  if (cookies === "") {
    // if no cookies, close connection
    socket.destroy();
    return;
  }
  ...
  cookiesplittedPairs.forEach((pair) => {
    ...
    cookieObj[decodeURIComponent(pair[0].trim())] = decodeURIComponent(
      pair[1].trim(),
    );
  });

  if (!cookieObj["srtoken"]) {
    // if no srtoken cookie, close connection
    socket.destroy();
    return;
  }
  ...
  if (!srtoken) {
    // if srtoken cookie is empty, close connection
    socket.destroy();
    return;
  }
  ...
  if (!find_user) {
    // if token is not in database, close connection
    socket.destroy();
    return;
  }
...
server_wss_CLIENT_GM.listen(PORT_CLIENT_GM, () => {
  console.log(
    `SERVER_WSS_CLIENT_GM is running on http://${LOCALHOST}:${PORT_CLIENT_GM}`,
  );
});
