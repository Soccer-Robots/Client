<template>
  ...
  >
    <!-- Navigation -->
    <nav
      class="sticky top-0 z-40 border-b border-white/10 bg-[#f96c00] px-4 py-3 shadow-lg sm:px-6"
    >
      <div
    ...
    <main class="mx-auto max-w-[1500px] px-4 py-6 sm:px-6 lg:py-8">
      <!-- Status row -->
      <section
        class="mb-6 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-white/10 bg-white/10 px-5 py-4 shadow-lg backdrop-blur-sm"
      >
        <div>
          <p class="text-xs font-bold uppercase tracking-[0.25em] text-orange-300">
            Live Soccer Robots
          </p>
        ...
        <div class="flex flex-wrap items-center gap-2">
          <span
            ...
            :class="gameFeedStatusClass"
          >
            <span class="h-2 w-2 rounded-full bg-current" />
            Game feed: {{ gameFeedStatus }}
          </span>

          <span
            v-if="isLoggedIn && !isInGame"
            class="inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-xs font-bold"
